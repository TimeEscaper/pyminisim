...
class DoMPCController:
    ...
    def __init__(self, dt: float, goal: np.ndarray, sim: Simulation,
                 obstacles: np.ndarray, horizon: int = 80):
        self._dt = dt
        self._sim = sim
        ...
        self._mpc.bounds['lower', '_u', 'u_omega'] = -np.deg2rad(30.)
        self._mpc.bounds['upper', '_u', 'u_omega'] = np.deg2rad(30.)
...
OBSTACLES = np.array([[3, 3, 0.8]])


def create_sim() -> Tuple[Simulation, Renderer]:
    robot_model = UnicycleRobotModel(initial_pose=np.array([1., 3.85, 0.0]),
...
def main():
    ...
    controller = DoMPCController(dt=0.1, goal=np.array([4., 1., 0.]), sim=sim, obstacles=OBSTACLES)
